...
3. Setze deinen OpenAI API-Schlüssel als Umgebungsvariable:
   ...
   $env:OPENAI_API_KEY="dein-api-schlüssel-hier"
   
   # Auf Windows (Eingabeaufforderung)
   set OPENAI_API_KEY=dein-api-schlüssel-hier
   
   # Auf Linux/Mac
   export OPENAI_API_KEY="dein-api-schlüssel-hier"
   ```

   Oder füge ihn deinem Shell-Profil (`~/.bashrc`, `~/.zshrc` usw.) für Persistenz hinzu.
...
- `--raw` (optional): Nur das Transkript drucken, ohne Zusammenfassung
  - Nützlich für Debugging oder wenn du nur das Transkript benötigst

- `--api-key` (optional): OpenAI API-Schlüssel (Standard ist die Umgebungsvariable `OPENAI_API_KEY`)
  - Verwende dies, wenn du den Schlüssel lieber über die Befehlszeile anstelle der Umgebungsvariable übergeben möchtest
...
python -m youtube_summarizer.cli --url "https://youtu.be/dQw4w9WgXcQ" --api-key "dein-schlüssel-hier"
```
...
Oder führe spezifische Testdateien aus:
...
- Die Nutzung der API kann Kosten verursachen, abhängig von deinem OpenAI-Plan
...
- Stelle sicher, dass die URL eine gültige YouTube-Video-URL ist
- Versuche verschiedene URL-Formate (youtube.com/watch, youtu.be usw.)

### "Transkripte sind deaktiviert" Fehler
- Das Video hat möglicherweise keine verfügbaren Untertitel
- Versuche, das `--raw`-Flag zu verwenden, um zu sehen, ob ein Transkript verfügbar ist

### "OpenAI API-Schlüssel nicht bereitgestellt" Fehler
- Setze die Umgebungsvariable `OPENAI_API_KEY`
- Oder verwende die Befehlszeilenoption `--api-key`

### Modul nicht gefunden Fehler
- Stelle sicher, dass alle Abhängigkeiten installiert sind: `pip install -r requirements.txt`
- Überprüfe, ob du aus dem richtigen Verzeichnis ausführst
